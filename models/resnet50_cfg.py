...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, cfg, stride=1, downsample=None):
        ...
        self.select.indexes.data = torch.cat([torch.ones(cfg[0]), torch.zeros(inplanes-cfg[0])], 0)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, cfg, num_classes=1000, imagenet=False):
        self.inplanes = 64
...
def resnet50_official(depth=50, dataset='ImageNet', imagenet=False, cfg=None, pretrained=False,num_classes=1000):
    ...
    model = ResNet(Bottleneck, [3, 4, 6, 3], cfg, imagenet=imagenet, num_classes=num_classes)
    if pretrained:
