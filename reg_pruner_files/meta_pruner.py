...
from dlth_utils import strdict_to_dict
...
class MetaPruner:
    ...
    def get_pr(self):
        '''获取每层的修剪比例。'''
        self.pr = {}  # 初始化一个字典来存储每层的修剪比例

        # 检查是否有阶段性修剪比例设置
        if self.args.stage_pr:  # 如果stage_pr有设置
            assert self.args.base_pr_model is None  # 确保没有基础修剪模型提供，防止配置冲突
            # 根据不同的层索引方式选择修剪比例计算方法
            if self.args.index_layer == 'numbers':  # 旧方法，基于数字索引，已废弃
                # 根据架构选择适当的函数来获取修剪比例
                get_layer_pr = self._get_layer_pr_vgg if self.is_single_branch(
                    self.args.arch) else self._get_layer_pr_resnet
                for name, m in self.model.named_modules():
                    if isinstance(m, self.learnable_layers):
                        self.pr[name] = get_layer_pr(name)
            elif self.args.index_layer == 'name_matching':  # 基于名称匹配的新方法
                for name, m in self.model.named_modules():
                    if isinstance(m, self.learnable_layers):
                        self.pr[name] = self._get_pr_by_name_matching(name)
        else:
            # 如果没有阶段性修剪比例，则加载基础修剪模型的修剪比例
            assert self.args.base_pr_model  # 确保有基础修剪模型提供
            state = torch.load(self.args.base_pr_model)  # 加载修剪模型的状态
            ...
            for k in self.pruned_wg_pr_model:
                ...
                # 计算修剪比例，即被修剪的权重数除以总权重数（被修剪的加上保留的）
                self.pr[k] = float(n_pruned) / (n_pruned + n_kept)

            self.logprint("==> Load base_pr_model successfully and inherit its pruning ratio: '{}'".format(
