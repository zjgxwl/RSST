...
def prune_model_custom_fillback(model, mask_dict, conv1=False, criteria="remain", train_loader=None, init_weight=None, trained_weight=None, return_mask_only=False, strict=True, fillback_rate=0.0):
    ...
    try:
        ...
        for key in list(trained_weight.keys()):
            if ('mask' in key):
                # 应用掩码更新权重 原始权重与掩码相乘 得到不带后缀的（eg：layer1.weight）正式权重
    ...
    for i, (name, m) in enumerate(model.named_modules()):
        ...
        if isinstance(m, nn.Conv2d):
            ...
            if (name == 'conv1' and conv1) or (name != 'conv1'):
                ...
                # 计算掩码非零元素的比例（这里未使用，已被注释）
                # sparsity = torch.sum(mask) / mask.numel()
                # 计算非零通道的实际数量 激活权重数/单个通道的权重总数
                '''
                假设一个卷积层有3个通道，每个通道有100个权重，其掩码可能如下：

                第一个通道的掩码：50个1，50个0（50个权重被保留）
                第二个通道的掩码：70个1，30个0（70个权重被保留）
                第三个通道的掩码：80个1，20个0（80个权重被保留）
                则：
                
                count = [50, 70, 80]
                count.sum() = 200
                每个通道的权重总数 = 100
                所以：num_channel = (200 / 100).item() = 2.0
 
                '''
                num_channel = (count.sum().float() / mask.shape[1]).item()
                # 打印相关信息，用于调试
                print(num_channel)  # 打印非零通道数量
                print(mask.shape[0])  # 打印总通道数
                print(fillback_rate)  # 打印填充率
                print(mask.shape[0] - num_channel)  # 打印零通道数量
                print((mask.shape[0] - num_channel) * fillback_rate)  # 打印计划恢复的通道数量
                # 计算考虑填充率后的非零通道数量
                ...
                # 根据选择的剪枝标准执行不同操作
                if criteria == 'remain':
                    print(mask.shape[0] - int_channel)  # 打印剪枝后的通道数量
                    # 计算剪枝阈值，获取阈值以下的通道索引
